<template>
  <li @click=showDeleteContainer class="product__item">
    <div  @click=deleteProduct :class="{
      'product__delete-container': true,
      'visually-hidden': showDelete
      }"
      >
      <svg
        class="product__delete-svg"
        viewBox="0 0 16 16"
        fill="none"
      >
    ...
    <img class="product__item-photo" src="../assets/Card/card__photo.png" />
    <div class="product__item-info">
      <h3 class="product__item-title">{{ todo.title }}</h3>
      <p class="product__item-description">{{ todo.description }}</p>
      <p class="product__item-price">{{ todo.price }} рублей</p>
    </div>
  </li>
</template>
...
export default {
  ...
  methods: {
    deleteProduct () {
      this.$emit('delete-product', this.todo.id)
    },
    showDeleteContainer () {
      this.showDelete ? this.showDelete = false : this.showDelete = true
    }
  }
}
</script>
